...
El usuario indicará cuál será la altura del triángulo ingresando un número entero positivo. Para el ejemplo anterior, la altura ingresada fue de 5.
    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            espacios = numeroIngresado;
            for (int i = 0; i < numeroIngresado; i++)
            {
                Console.WriteLine("".PadLeft(espacios, ' ') + "".PadRight(contador, asterisco));//primero que imprime , el "", despues el len que tendra la cadena y por ultimo con que rellenar la cadena
                contador = contador + 2;
                espacios = espacios - 1;
            }
        }
